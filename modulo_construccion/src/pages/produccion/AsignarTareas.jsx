...
function AsignarTareas() {
    ...
    const loadInitialData = async () => {
        try {
            // Cargamos servicios en paralelo para mejorar velocidad
            const [
                cuadrillasRes,
                actividadesRes,
                zonasRes,
            ...
            // 1. Cuadrillas
            let c = []
            if (cuadrillasRes.status === 'fulfilled') c = cuadrillasRes.value
            else console.error('Error cargando cuadrillas:', cuadrillasRes.reason)

            // 2. Actividades
            let a = []
            if (actividadesRes.status === 'fulfilled') a = actividadesRes.value
            else console.error('Error cargando actividades:', actividadesRes.reason)

            // 3. Zonas
            let z = []
            if (zonasRes.status === 'fulfilled') z = zonasRes.value
            else console.error('Error cargando zonas:', zonasRes.reason)

            // 4. Proyecto Info
            let p = null
            if (proyectoRes.status === 'fulfilled') p = proyectoRes.value
            else console.error('Error cargando proyecto:', proyectoRes.reason)

            // 5. Estados de Pago
            let todosLosEPs = []
            if (epsRes.status === 'fulfilled') todosLosEPs = epsRes.value
            else console.error('Error cargando EPs:', epsRes.reason)

            // 6. Tareas
            let t = []
            if (tareasRes.status === 'fulfilled') t = tareasRes.value
            else console.error('Error cargando tareas:', tareasRes.reason)
